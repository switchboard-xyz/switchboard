...
- [ ] Create user and request account
- [ ] Request randomness
...
In our example we first make a `house_init` instruction to initialize the top level program state and store the expected Function for our program.
...
Our function is verify simple because we are able to generate randomness within the enclave. **All you need to generate randomness is one line ` Gramine::read_rand(&mut bytes)`!**
