...
permanent user accounts for your function or one-time-use `FunctionRequestAccount`'s. This design decision
...
permanent request account for each user since they can only spin the roulette wheel one at a time.
...
Besides `user_init`, we will need two instructions: `user_guess` to initiate the request then `user_settle` to finish the request.
