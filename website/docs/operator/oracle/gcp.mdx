...
and to date, it has be able to run on an auto pilot GKE cluster without issue.
...
The `oracles/helm-deployment` directory in the
[Switchboard-V2 SDK](https://github.com/switchboard-xyz/switchboard-v2) contains
a bash script to walk through the GCP setup and output the required variables to
an env file. This script is provided as a convenience tool, you should
understand all of the commands in the script before running. To automate the GCP
