...
TODO: High level overview of how Switchboard is structured

- Queues are partitions networks
- Queues control the security permissions of its participants
- Queues can be governed by a DAO, multisig, or central authority
- Queues provide the flexibility to allow anyone to run their own oracle network
...
Oracle queue's currently support update requests from the following resource
types:
