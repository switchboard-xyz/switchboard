...
export function readCliCommands(projectRoot: string): ParsedCommand[] {
...
export function readCliReadme(projectRoot: string): string {
  const cliPath = path.join(projectRoot, "cli");
  const readmePath = path.join(cliPath, "README.md");
  ...
  const startIndex = readmeLines.findIndex((l) => l.startsWith("## Testing"));
  return readmeLines.slice(0, startIndex).join("\n");
}

// export function convertToParsedCommands(commands: Commands): ParsedCommand[] {
//   const parsedCommands: ParsedCommand[] = [];

//   for (const key in commands) {
//     // @ts-ignore
//     const commandTopicObj = commands[key];

//     for (const topic in commandTopicObj) {
//       const commands = commandTopicObj[topic];

//       if (Array.isArray(commands)) {
//         for (const command of commands) {
//           const parsedCommand: ParsedCommand = {
//             command: `sbv2 ${key} ${topic}`,
