...
import { replaceLinkTokens } from "../modules/typedocs/utils/markdown";
  ...
  ast.forEach((token) => {
    ...
    switch (token.type) {
      ...
      case "codespan":
        ...
        elements.push(
          <MDX.code key={counter}>{token.raw.slice(1, -1)}</MDX.code>
        );
        break;
      ...
      case "image":
        elements.push(
          <MDX.img key={counter} alt={token.title} src={token.href} />
        );
        break;
      ...
      default: {
        ...
        elements.push(
          <Comp key={counter}>
            {convertAstToElements(children) ?? innerText}
          </Comp>
        );
        break;
      }
...
export function Markdown({ content }: MarkdownProps) {
  ...
  return (
    <div className="tsd-markdown markdown">{convertAstToElements(ast)}</div>
  );
}
