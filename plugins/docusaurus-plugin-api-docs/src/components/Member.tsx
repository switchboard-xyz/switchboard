...
import { hasOwnDocument } from "../modules/typedocs/utils/visibility";
...
export function Member({ id }: MemberProps) {
  ...
  if (reflection.signatures) {
    ...
    "target" in reflection &&
    (reflection as JSONOutput.ReferenceReflection).target
  ) {
    content = (
      <MemberReference
        reflection={reflection as JSONOutput.ReferenceReflection}
      />
    );
  } else {
