...
import type { HierarchyNode } from "../modules/typedocs/utils/hierarchy";
        ...
        <li key={type.type + String(i)}>
          {tree.isTarget ? (
            <em className="tsd-target">
              {type.type === "reference" ? type.name : "UNKNOWN"}
            </em>
          ) : (
            <Type type={type} />
          )}

          {i === tree.types.length - 1 && tree.next && (
            <Hierarchy tree={tree.next} />
          )}
        </li>
      ))}
    </ul>
