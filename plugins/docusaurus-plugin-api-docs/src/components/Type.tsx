...
export function Type({ needsParens = false, type: base }: TypeProps) {
  ...
  switch (String(base.type)) {
    ...
    case "reference": {
      ...
      const genericClass =
        ref?.id && !ref.sources ? "tsd-signature-type-generic" : "";
      ...
      return (
        <>
          {ref?.permalink ? (
            ...
            <span className={`tsd-signature-type ${genericClass}`}>
              {type.name}
            </span>
          )}
    ...
    case "reflection": {
      ...
      if (decl?.signatures && decl.signatures.length === 1) {
        return (
          <MemberSignatureTitle hideName useArrow sig={decl.signatures[0]} />
        );
      }
        ...
        <>
          ...
          {type.tail.map((t, i) => (
            <React.Fragment key={i}>
              ...
              <span className="tsd-signature-symbol">{"}"}</span>
              {typeof t[1] === "string" && (
                <span className="tsd-signature-type">{t[1]}</span>
              )}
            </React.Fragment>
          ))}
          <span className="tsd-signature-symbol">`</span>
