...
import { createHierarchy } from "../modules/typedocs/utils/hierarchy";
  ...
  return (
    <>
      {isCommentWithModifiers(reflection.comment) && (
        <CommentBadges comment={reflection.comment} />
      )}
      {hasComment(reflection.comment) && (
        <Comment root comment={reflection.comment} />
      )}
      ...
      {(("extendedBy" in reflection &&
        reflection.extendedBy &&
        reflection.extendedBy.length > 0) ||
