...
function shouldEmit(projectRoot: string, tsconfigPath: string) {
  ...
  return result.projectReferences && result.projectReferences.length > 0
    ? "docs"
    : "none";
}
...
export function loadPackageJsonAndDocs(
  ...
  return {
    packageJson: JSON.parse(
      fs.readFileSync(path.join(currentDir, pkgFileName), "utf8")
    ) as {
      name: string;
      version: string;
    },
...
export function addMetadataToReflections(
  ...
  if (project.children) {
    ...
    project.children = project.children.map((child) => {
      ...
      const childSlug = kindSlugPart
        ? `/${kindSlugPart}/${child.name}`
        : `#${child.name}`;
...
function mergeReflections(
  base: JSONOutput.ProjectReflection,
  next: JSONOutput.ProjectReflection
) {
...
function extractReflectionModules(
  ...
  if (isSinglePackage) {
    const hasNoModules = project.children?.every(
    ...
    packageConfigs.some((cfg) =>
      Object.entries(cfg.entryPoints).some(([importPath, entry]) => {
        ...
        if (!packages[cfg.packagePath]) {
          const { packageJson, readmePath, changelogPath } =
          ...
          packages[cfg.packagePath] = {
            ...
            packageVersion:
              (versioned && cfg.packageVersion) || packageJson.version,
            readmePath,
            changelogPath,
          };
        ...
        if (existingEntry) {
          if (isUsingDeepImports) {
            ...
            console.error(
              `Entry point ${urlSlug} already defined. How did you get here?`
            );
          }
        } else {
...
export function formatPackagesWithoutHostInfo(
