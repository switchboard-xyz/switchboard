...
function getDefaultLastVersionName(versionNames: string[]): string {
  ...
  return (
    versionNames.find((versionName) => versionName !== CURRENT_VERSION_NAME) ??
    ""
  );
}
...
function createVersionMetadata({
  ...
  const versionLabel =
    versionOptions.label ?? versionName === CURRENT_VERSION_NAME
      ? "Next"
      : versionName;
  let versionPathPart =
    versionOptions.path ?? versionName === CURRENT_VERSION_NAME
      ? "next"
      : versionName;
  ...
  const versionPath = normalizeUrl(
    (() => {
      const paths = [context.siteConfig.baseUrl];
      options.routeBasePath && paths.push(options.routeBasePath);
      paths.push(versionPathPart);
      return paths;
    })()
  );
