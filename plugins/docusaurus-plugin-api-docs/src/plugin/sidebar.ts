  ...
  sortedGroups.forEach((group, groupIndex) => {
    ...
    items.push({
      ...
      items: children.map((id, index) => {
        ...
        const previousId =
          index === 0
            ? getLastItemInGroup(groupIndex - 1)
            : children[index - 1];
        const nextId =
...
export function extractReflectionSidebar(
  pkg: JSONOutput.ProjectReflection
): SidebarItem[] {
  return pkg.groups
    ? groupSidebarItems(createReflectionMap(pkg.children), pkg.groups)
    : [];
}
...
export function extractSidebar(
  ...
  const items: SidebarItem[] = packages.map((pkg) => {
    ...
    const indexHref =
      pkg.entryPoints.find((entry) => entry.index)?.reflection.permalink ?? "";
