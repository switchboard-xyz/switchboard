...
impl Gramine {
    ...
    pub fn generate_quote(user_data: &[u8]) -> std::result::Result<Vec<u8>, SbError> {
        match fs::metadata("/dev/attestation/quote") {
            Ok(_) => (),
            Err(_) => return Err(SbError::SgxError),
        }
    ...
    /// # Returns
    /// Error on failure.
    pub fn read_rand(buf: &mut [u8]) -> std::result::Result<(), SbError> {
        // https://gramine.readthedocs.io/en/latest/devel/features.html#randomness
